...
BEGIN
  ...
  FOR user_record IN SELECT id FROM auth.users LOOP
    -- Create sample projects for each user using valid current_phase values and compatible status
    ...
    (gen_random_uuid(), 'E-commerce Website Redesign', 'Complete redesign of company e-commerce platform with modern UI/UX and mobile optimization', 'active', 'high', '2024-01-15', '2024-06-30', 75000, 65, 'execution', user_record.id, user_record.id),
    (gen_random_uuid(), 'Mobile App Development', 'Native iOS and Android app for customer service and order tracking', 'active', 'urgent', '2024-02-01', '2024-08-15', 120000, 35, 'planning', user_record.id, user_record.id),
    (gen_random_uuid(), 'Data Analytics Dashboard', 'Business intelligence dashboard for real-time analytics and reporting', 'active', 'medium', '2024-03-01', '2024-09-30', 45000, 80, 'execution', user_record.id, user_record.id),
    (gen_random_uuid(), 'AI Chatbot Integration', 'Implement AI-powered customer support chatbot with natural language processing', 'completed', 'low', '2023-11-01', '2024-01-31', 30000, 100, 'closed', user_record.id, user_record.id),
    (gen_random_uuid(), 'Cloud Infrastructure Migration', 'Migrate existing systems to AWS cloud infrastructure with improved security and scalability', 'on_hold', 'high', '2024-04-01', '2024-12-31', 95000, 25, 'bidding', user_record.id, user_record.id);

    -- Get the newly created project IDs for this user
    FOR project_id IN SELECT id FROM projects WHERE created_by = user_record.id AND name IN ('E-commerce Website Redesign', 'Mobile App Development', 'Data Analytics Dashboard', 'AI Chatbot Integration', 'Cloud Infrastructure Migration') AND created_at > now() - interval '1 minute' LOOP
      -- Add project members
      INSERT INTO project_members (project_id, user_id, role, joined_at) VALUES
      (project_id, user_record.id, 'admin', now())
      ON CONFLICT (project_id, user_id) DO NOTHING;

      -- Create sample tasks based on project name
      CASE 
