-- Temporarily disable the project lifecycle trigger to avoid the enum issue
DROP TRIGGER IF EXISTS initialize_project_lifecycle_trigger ON public.projects;

-- Insert 6 sample construction projects for Ontario
...
-- Re-enable the trigger (only for new projects)
CREATE OR REPLACE TRIGGER initialize_project_lifecycle_trigger
  AFTER INSERT ON public.projects
  FOR EACH ROW
  EXECUTE FUNCTION public.initialize_project_lifecycle();